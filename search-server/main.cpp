...
vector<Document> FindTopDocuments(const string& raw_query) const {
    ...
    vector<Document> FindTopDocuments(const string& raw_query, DocumentStatus status) const {
        auto status_predicate = [status](int document_id, DocumentStatus doc_status, int rating) {
            ...
            if (word_to_document_freqs_.count(word) > 0) {
                ...
                for (const auto [document_id, term_freq] : word_to_document_freqs_.at(word)) {
                    if (predicate(document_id, documents_.at(document_id).status, documents_.at(document_id).rating)) {
                        document_id_relevance[document_id] += term_freq * inverse_document_freq;
                    }
                }
            }
        }
        ...
        for (const string& word : query.minus_words) {
            if (word_to_document_freqs_.count(word) > 0) {
                for (const auto [document_id, _] : word_to_document_freqs_.at(word)) {
                    document_id_relevance.erase(document_id);
                }
            }
        }

        for (const auto [document_id, relevance] : document_id_relevance) {
